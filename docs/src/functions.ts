 ...
 * 3-point polyline 
 */
export const polyline = (input: number, midpointX: number, midpointY: number) => {
 ...
 */
export function inflectionThroughPoint<
  Fn extends typeof functions[keyof typeof functions]
>(input, x, y, fn: Fn, ...args: Omit<Parameters<Fn>, "input">) {
  if (input <= x) {
